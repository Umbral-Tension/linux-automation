...
if __name__ == '__main__':
    ...
    args = parser.parse_args("--dry-run".split(" "))
    ...
    current = opath.join(backups, "current")
    if not opath.exists(current) or args.full:
        newbackup = f'{backups}/{timestamp}_(Full)'
    else:
        newbackup = f'{backups}/{timestamp}_(Incremental)'
    


    # Build and run an rsync command to create a new backup. Send stdout and stderr to file.
    rs_opts = []
    rs_opts.extend(f'--debug=FILTER --itemize-changes --human-readable --progress --archive --delete --partial --log-file="{opath.join(backups, "temp rsync log")}"'.split(" ")) 
    ...
    print("running rsync as follows (unquoted spaces here are not actually passed to rsync that way): ") 
    rsync_cmd = f'rsync {" ".join(rs_opts)} {" ".join(src)} "{newbackup}" 2> "{backups}/temp rsync stderr" | tee "{backups}/temp rsync stdout"'
    test(shlex.split(rsync_cmd))
